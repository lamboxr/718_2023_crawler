...
class M3U8_Download():
    def download(self, m3u8_url, fragments_cache_dir):
        try:
            ...
            if resp.status_code == 200:
                ...
                # 第一层
                file_lines, m3u8_url = self.handle_level_1(all_content, m3u8_url)

                # 第二层
                total_count = self.calc_fragments_num(file_lines, m3u8_url)

                # 判断命令字符数超限
                merge_command = self.gen_merge_command(fragments_cache_dir, total_count)
                logger.debug("pre gen merge command: %s" % merge_command)
                if len(merge_command) > constraints.cmd_max_char_num:
                    return DownloadCode._COMMAND_TOO_LONG.value, ''
    ...
    def calc_fragments_num(self, file_lines, m3u8_url):
        total_count = 0
    ...
    def handle_level_1(self, all_content, m3u8_url):
        # 第一层
    ...
    def merge_file(self, fragments_folder_path, total_count):
    ...
    def gen_merge_command(self, fragments_folder_path, fragment_amt):
        input_files = []
        for i in range(fragment_amt):
            input_files.append(str(i))
        input_file_str = '|'.join(input_files)
        output_file = fragments_folder_path + '.mp4'
        command = 'cd {} && ffmpeg -i "concat:{}" -acodec copy -vcodec copy -absf aac_adtstoasc {}'.format(
            fragments_folder_path, input_file_str, output_file)
        return command
